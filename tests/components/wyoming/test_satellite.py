...
async def test_announce_with_preannounce(
    ...
    with (
        ...
        for temp_file in (temp_wav_file, temp_preannounce_wav_file):
        ...
        # If the preannounce sound was played then total time should be
        # 1000 milliseconds minus some tolerance for encoding and rounding
        assert mock_client.tts_audio_chunk is not None
        assert mock_client.tts_audio_chunk.timestamp > 900
